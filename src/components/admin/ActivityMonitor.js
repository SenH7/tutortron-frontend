// src/components/admin/ActivityMonitor.js
const ActivityMonitor = ({ activities, onFlagActivity, onViewDetails }) => {
  const [filter, setFilter] = useState('all');
  
  const filteredActivities = activities.filter(activity => {
    if (filter === 'flagged') return activity.flagged;
    if (filter === 'normal') return !activity.flagged;
    return true;
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        ...
        >
          Normal ({activities.filter(a => !a.flagged).length})
        </button>
      </div>
...
export { UserDetailModal, ActivityMonitor };